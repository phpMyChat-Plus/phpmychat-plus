...
class upload {
    ...
    function upload($file, $lang = 'en_GB') {
        ...
        $this->translation['uploaded_missing']            = 'File upload error (no file was specified).';
        ...
        $this->lang               = $lang;
        if ($this->lang != 'en_GB' && file_exists('plugins/uploader/lang/class.upload.' . $lang . '.php')) {
            $translation = null;
            include('plugins/uploader/lang/class.upload.' . $lang . '.php');
    ...
    function process($server_path = null) {
        ...
        if ($this->uploaded) {
            ...
            if ($this->file_src_size > $this->file_max_size ) {
                ...
                $this->error = $this->sprintf(translate('file_too_big'),$this->file_src_size);
            } else {
            ...
            if ($this->file_is_image) {
                if (is_numeric($this->image_src_x) && is_numeric($this->image_src_y)) {
                    ...
                    if (!is_null($this->image_max_width) && $this->image_src_x > $this->image_max_width) {
                        $this->processed = false;
                        $this->error = $this->sprintf(translate('image_too_wide'),$this->image_max_width);
                    }
                    if (!is_null($this->image_min_width) && $this->image_src_x < $this->image_min_width) {
                        $this->processed = false;
                        $this->error = $this->sprintf(translate('image_too_narrow'),$this->image_min_width);
                    }
                    if (!is_null($this->image_max_height) && $this->image_src_y > $this->image_max_height) {
                        $this->processed = false;
                        $this->error = $this->sprintf(translate('image_too_high'),$this->image_max_height);
                    }
                    if (!is_null($this->image_min_height) && $this->image_src_y < $this->image_min_height) {
                        $this->processed = false;
                        $this->error = $this->sprintf(translate('image_too_short'),$this->image_min_height);
                    }
                    if (!is_null($this->image_max_ratio) && $ratio > $this->image_max_ratio) {
                        $this->processed = false;
                        $this->error = $this->sprintf(translate('ratio_too_high'),$this->image_max_ratio);
                    }
                    if (!is_null($this->image_min_ratio) && $ratio < $this->image_min_ratio) {
                        $this->processed = false;
                        $this->error = $this->sprintf(translate('ratio_too_low'),$this->image_min_ratio);
                    }
                    if (!is_null($this->image_max_pixels) && $this->image_src_pixels > $this->image_max_pixels) {
                        $this->processed = false;
                        $this->error = $this->sprintf(translate('too_many_pixels'),$this->image_max_pixels);
                    }
                    if (!is_null($this->image_min_pixels) && $this->image_src_pixels < $this->image_min_pixels) {
                        $this->processed = false;
                        $this->error = $this->sprintf(translate('not_enough_pixels'),$this->image_min_pixels);
                    }
                } else {
