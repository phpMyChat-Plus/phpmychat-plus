...
define("L_EMAIL_VAL_32", "%s（%s）で利用できるあなたの登録データは次のとおりです："); //chat name at chaturl
define("L_EMAIL_VAL_4", "%s（%s）で利用できるアカウントの登録が完了しました"); //chat name at chaturl
define("L_EMAIL_VAL_41", "%sとして%sで利用する重要なアカウント情報を変更しました（影響するアカウント：%s）。"); //chat name at chaturl (username)
...
define("L_HELP_CMD_22", "この部屋にビープ音を鳴らし、オプションによっては指定した文字を表示します。<br />使用法：<br />－従来の方法：「/buzz」または「/buzz 表示したい文字列」－管理パネルで定義されているデフォルトサウンドを鳴らします。<br />－拡張した方法：「/buzz ~サウンド名」または「/buzz ~サウンド名 表示したい文字列」－soundフォルダからサウンド名.wavファイルを見つけて鳴らします。このとき、サウンド名の前に「~」を付け忘れないようにし、また、拡張子の.wavは不要です（.wavファイルのみ有効）。<br />デフォルトでは、モデレータ／管理者のみ有効なコマンドです。");
