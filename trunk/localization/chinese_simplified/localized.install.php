...
define("L_P3_HINT2", "这里是你的配置结果，需要自行复制贴到 \"config/config.lib.php\" 档案。 请从下面的讯息框选择所有文字，将它复制并贴到您喜欢的文本编辑器 (例如. Notepad++)。 将该文件储存为 config.lib.php (确保类型是所有类型，不是纯文字文件) 并把文件上传到您的网站服务器上 \"config\" 目录中。");
...
define("L_P2_FORM01", "资料库-主机 用于 phpMyChat");
...
define("L_P2_FORM12", "重命名你的管理员日志记录文件夹");
define("L_P2_FORM13", "如果您打算使用集成模块 phpMyChat PHPNuke的或PHPBB，组态配置表必须调用 \"c_config\" 跟注册用户表必须调用 \"c_reg_users\"！");
...
define("L_P4_FORM2", "可选，你还可以安装一个(机器人，虚拟用户)到您的聊天室，因此它可以增加一些乐趣在你的聊天室。 可以做到这一点，但是这是最佳时间来做这件事。 如果你点击下面的链接，请不要阻止脚本在新的弹出窗口中运行！");
define("L_P4_FORM3", "安装机器人");
?>