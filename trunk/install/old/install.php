		...
		if (APP_VERSION != "")
		{
			...
			else
			{
				switch (APP_VERSION)
				{
					case "1.94":
					{
						if (APP_MINOR != "" && strstr(APP_MINOR,"RC4")) $kind = "194-RC4";
						elseif (APP_MINOR != "" && strstr(APP_MINOR,"RC3")) $kind = "194-RC3";
						elseif (APP_MINOR != "" && strstr(APP_MINOR,"RC2")) $kind = "194-RC2";
						elseif (APP_MINOR != "" && strstr(APP_MINOR,"beta6")) $kind = "194-beta6";
						elseif (APP_MINOR != "") $kind = "194-beta";
						else $kind = "194";
					}
					break;
					case "1.93":
...
if ( $p == 4 )
{
	...
	if ( $error == "" )
	{
		...
		{
			include ( "database/mysql_upgrade_plus_1.94-RC4.php" );
			mysql_query ( $LOG_DIR_NN );
		} elseif ( $kind == "194-RC3" )
		{
			include ( "database/mysql_upgrade_plus_1.94-RC3.php" );
			mysql_query ( $LOG_DIR_NN );
		} elseif ( $kind == "194-RC2" )
		{
			include ( "database/mysql_upgrade_plus_1.94-RC2.php" );
			mysql_query ( $LOG_DIR_NN );
		} elseif ( $kind == "194-beta6" )
		{
			include ( "database/mysql_upgrade_plus_1.94-beta6.php" );
			mysql_query ( $LOG_DIR_NN );
...
if ( ! $fh = fopen ( $ChatPath."config/config.lib.php", "w" ) )
{
  ...
  if ( ! fputs ( $fh, '<?php'.$lfeed ) )
  {
		...
		fputs ( $fh, '$USE_FLAGS					= $row[201];'.$lfeed );
		fputs ( $fh, '$SHOW_FLAGS					= $row[202];'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		...
		fputs ( $fh, '	}'.$lfeed );
		fputs ( $fh, '	if($L_WELCOME_SND != "")'.$lfeed );
		fputs ( $fh, '	{'.$lfeed );
		...
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, '// Colors and Tags for chat text by Ciprian'.$lfeed );
		fputs ( $fh, 'define("C_ALLOW_TEXT_COLORS", $ALLOW_TEXT_COLORS);'.$lfeed );
		fputs ( $fh, 'define("C_TAGS_POWERS", $TAGS_POWERS);'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, '// MathJax LaTeX formulas rendering in chat by Ciprian'.$lfeed );
		fputs ( $fh, 'define("C_ALLOW_MATH", $ALLOW_MATH);'.$lfeed );
		fputs ( $fh, 'define("C_SRC_MATH", $SRC_MATH);'.$lfeed );
		fputs ( $fh, ''.$lfeed );
		fputs ( $fh, '// GeoIP Country flags Mod based on MaxMind GeoIPLite2 by Ciprian'.$lfeed );
		fputs ( $fh, 'define("C_USE_FLAGS", $USE_FLAGS);'.$lfeed );
		fputs ( $fh, 'define("C_SHOW_FLAGS", $SHOW_FLAGS);'.$lfeed );
		fputs ( $fh, '?>' );
...
<?php if ( $error == "" && $error3 == "" ) { ?>
	<p align="center">
		...
		<option value="194-RC4"<?php if ($kind=="194-RC4") echo " selected" ?>><?php echo sprintf(L_P1_OP02,"phpMyChat-Plus 1.94-RC4") ?></option>
		<option value="194-RC3"<?php if ($kind=="194-RC3") echo " selected" ?>><?php echo sprintf(L_P1_OP02,"phpMyChat-Plus 1.94-RC3") ?></option>
		<option value="194-RC2"<?php if ($kind=="194-RC2") echo " selected" ?>><?php echo sprintf(L_P1_OP02,"phpMyChat-Plus 1.94-RC2") ?></option>
		<option value="194-beta6"<?php if ($kind=="194-beta6") echo " selected" ?>><?php echo sprintf(L_P1_OP02,"phpMyChat-Plus 1.94-beta6") ?></option>
