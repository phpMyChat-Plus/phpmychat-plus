...
while(list($key, $langname) = each($AvailableLanguages))
{
	$i++;
		...
		elseif ($langname == "english" && L_ORIG_LANG_EN != "L_ORIG_LANG_EN") $FLAG_NAME = L_ORIG_LANG_EN.($langname != $L && L_LANG_EN != "L_LANG_EN" ? "/".L_LANG_EN : "");
		...
		if ($langname != $L) echo("<A HREF=\"$Action?L=${name}\" onMouseOver=\"window.status='".$FLAG_STATUS.".'; return true;\" Title=\"".$FLAG_OVER."\">");
