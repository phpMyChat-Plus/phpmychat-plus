<?php
// File : vietnamese/localized.tutorial.php - plus version (10.07.2007 - rev.6)
// Translated by Marshall <hellomarshal_lookatme@netzero.net>
...
</FONT><br /><I>Translated by Marshall - Mỹ Tho, VIỆT NAM, &copy; 2007<?php echo((date(Y)>"2007") ? "-".date(Y) : ""); ?></I></B></TD>
</TR>
</TABLE><br /><br />
<P><A NAME="top"></P>
...
<P>
Nếu bạn chưa đăng ký<?php if (!C_REQUIRE_REGISTER) echo(" and would like to"); ?>, hãy chọn tùy chọn đăng ký . Một cửa sổ nhỏ sẽ xuất hiện.
<P>
<UL>
	<LI>Đầu tiên, Tự mình tạo một tên đăng nhập và mật khẩu bằng việc việc viết nó vào những ô thích hợp<?php if (!C_EMAIL_PASWD) echo(" and a password"); ?> Tên đăng nhập là tên sẽ được hiển thị trong phòng tán gẫu thay cho bạn , bạn có thể dùng tên thật của mình nếu thích.Nó không thể chứa đựng những khỏang trống, những dấu phẩy hay những dấu xồ nguợc (\).Nó không thể chứa đựng "những từ chưởi thề".
...
<P>
<A NAME="modProfile"></A>Tất nhiên, Bạn có khả năng thay đổi thông tin <?php if (C_SHOW_DEL_PROF) echo("/delete"); ?> /Xóa bỏ những thông tin đã đề cập bằng 1 cái click. <?php echo((!C_SHOW_DEL_PROF ? "link" : "links")); ?>.<br />
<br /><P ALIGN="right"><A HREF="#top">Quay lại từ đầu</A></P>
<P>
<hr />
...
if (C_VERSION == "2")
{
	...
	<P>
	Tên của phòng không thể chứa đựng những dấu phẩy hay dấu xổ ngược (\). Không thể chứa đựng "Từ chưởi thề".).<?php if (C_NO_SWEAR) echo(" It cannot contain \"swear words\"."); ?>
	<br /><P ALIGN="right"><A HREF="#top">Quay lại từ đầu</A></P>
	<P>
	<hr />
	<?php
...
<P>
Gửi một lời nói tới phòng chat, đánh lời nói bạn vào khe dưới , gần chử help và nhất nút gửi để gửi đi hay đơn giản chỉ là cái Enter. Những thông báo sẽ được hiển thị tại cái hộp lớn hơn. Xin ghi nhớ "Chưởi thề" không được nói ra trong phòng chat.<br />
...
<P>
Để ra khỏi phòng tán gẫu, Đơn giản là click một lần trên "Exit" cách khác. Bạn có thể nhập các lệnh sau đây , tương tự như lời chat:<br />
/exit<br />
/bye<br />
/quit<br />
Những lệnh này được có thể hoàn thành với một thông báo sẽ được gửi trước khi bạn rời phòng chat.
<I>Ví dụ :</I> /quit see you soon!
<P>
sẽ gửi một thông báo "see you soon!" bạn đã thóat.
<br /><P ALIGN="right"><A HREF="#top">Quay lại từ đầu</A></P>
<P>
<hr />
...
<UL>
	...
	?>
	<LI>Từ <B>Notify lệnh</B> thông báo cho phép bạn tới hay ro khỏi tùy chọn bạn nhìn thấy những sự chú ý khi những người sử dụng khác vào hay ra khỏi phòng chuyện gẫu. Sự vắng mặt tùy chọn này <?php echo(C_NOTIFY ? "on" : "off"); ?> và những sự chú ý <?php echo(C_NOTIFY ? "will" : "won't"); ?> được nhìn thấy.<br />Đánh "/notify" không thêm một thứ nào khác .
	<P>
	<LI>Từ <B>Timestamp lệnh</B> cho bạn tới những lựa chọn trên hoặc tùy chọn bạn nhìn thấy Thời gian thông báo là thông báo đầy đủ trước đây từng cái và thời gian những người phục vụ trạng thái . Theo mặc định tùy chọn này trên . <?php echo(C_SHOW_TIMESTAMP ? "on" : "off"); ?>.<br />Đánh "/timestamp" không thêm một thứ nào khác .
	<P>
 ...
 <LI>Từ <B>Show lệnh</B> đây là giúp bạn hiện ra những lời nói trước hay tất cả các lời nói theo yêu cầu của bạn là số lượng. Đánh "/show n" hoặc "/last n" không có thêm một thứ nào khác ngòai số.
		...
		<P>
		will cause the 50 newest messages to be viewable on your screen. Nếu tất cả những thông báo không thể là được trình bày bên trong hộp thoại, hãy kéo thanh dọc để đọc để đọc những thông báo đó.</UL>
		<?php
	}
	else
	{
...
<P>
Một lần bên trong phòng tán gẫu, bạn có thể sử dụng công cụ popup ảnh mà ngồi trước hộp thọai <IMG SRC="images/helpOff.gif" WIDTH=30 HEIGHT=20 BORDER=0 ALT="Help"> bạn có thể đánh lệnh <B>"/help" hoặc "/?" commands</B> trong hộp thọai để cần trợ giúp.
<br /><P ALIGN="right"><A HREF="#top">Quay lại từ đầu</A></P>
<P>
<P>
<!-- Avatar System Start. -->
...
<P>Click vào ảnh đại diện trên một popup (hiện /whois lệnh). Bạn có thể đánh theo lệnh ảnh đại diện người sử dụng có thể kéo theo profile, (<A HREF="#whois">/whois command</A>).
...
if (C_HTML_TAGS_KEEP != "none")
{
	...
	<P>
	Để tạo ra một liên kết, đánh máy địa chỉ (không có bất kỳ HTML ). Link liên kết sẽ được tạo ra tự động.
	<br /><P ALIGN="right"><A HREF="#top">Quay lại từ đầu</A></P>
	<P>
	<P>
	<hr />
...
if (C_VERSION == "2")
{
	...
	?>
	nếu bạn là một người sử dụng được đăng ký. Bạn có thễ tạo ra một phòng với lệnh này tương ứng. Trừ phi bạn phải chỉ rõ kiểu đó: 0 thay cho riêng tư, 1 quần chúng (giá trị mặc định).
	<P>
	...
	<P>
	Tên phòng không thế chứa đựng dấu phẩy hay dấu xổ ngược (\). Nó không thể chứa đựng "từ chưởi thề" .<?php if (C_NO_SWEAR) echo(" It cannot contain \"swear words\"."); ?>
	<?php
}
?>
<br /><P ALIGN="right"><A HREF="#top">Quay lại từ đầu</A></P>
...
<P>
<FONT SIZE="+1"><B>Để thay đổi thông tin khi chat:</B></FONT>
<P>
...
<P>
<FONT SIZE="+1"><B>Gọi lại những lệnh thông báo hay nhửng gì bạn phụ thuộc:</B></FONT>
<P>
...
<P>
<FONT SIZE="+1"><B>Trả tới một người sử dụng một cách đặc biệt:</B></FONT>
<P>
...
<P>
<I>Ví dụ, nếu Jack có tên đăng nhậo là Jack thì:</I> /msg Jack Hi there, how are you?
<P>
...
<P>
<I>Ví dụ:</I> nếu Jack gửi tin nhắn "/me is drinking a coffee" khung thông báo sẽ hiễn thị "* ". "<B>* Jack</B> Jack is drinking a coffee".
<br /><P ALIGN="right"><A HREF="#top">Quay lại từ đầu</A></P>
<P>
<hr />
...
<P>
<I>Ví dụ: /announce The chat system is going down for maintenance tonight at 8pm.</I>
<br /><P ALIGN="right"><A HREF="#top">Quay lại từ đầu</A></P>
<P>
...
<P>
<I>Ví dụ: /room The meeting starts at 15 pm.</I> hoặc <I>/room * TNhững bắt đầu cuộc gặp tại 15 pm trong phòng Nhân viên.</I>
<br /><P ALIGN="right"><A HREF="#top">Quay lại từ đầu</A></P>
<P>
<hr />
