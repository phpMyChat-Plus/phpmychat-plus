<?php
// File : italian/localized.tutorial.php - plus version (26.05.2007 - rev.5)
...
<TR>
	<TD ALIGN="center"><FONT SIZE="+2" COLOR="GREEN"><B>- Guida in italiano per <?php echo(APP_NAME." - ".APP_VERSION.APP_MINOR); ?> -</B></FONT></TD>
</TR>
</TABLE><br /><br />
<P><A NAME="top"></P>
...
<P>
Se vi siete gi&agrave; registrati, potete semplicemente Entrare inserendo il vostro nome utente e la password. Dopo selezionate la stanza di chat dove vorreste entrare per poi premere il bottone '<?php echo(L_SET_14); ?>'.<br />
<?php
...
else
{
	?>
<P>
	Altrimenti potrete <A HREF="#register">registrarvi</A> prima oppure entrare semplicemente in una stanza ma il vostro nome utente non verr&agrave; riservato (un altro utente potr&agrave; utilizzare lo stesso username una volta che vi sarete scollegati).
	<?php
}
?>
<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
...
<P>
Se non siete ancora registrati<?php if (!C_REQUIRE_REGISTER) echo(" e volete farlo"); ?>, prego scegliete l'opzione di registrazione. Apparir&agrave; una piccola finestra pop-up.
<P>
<UL>
	<LI>Primo, create uno pseudonimo<?php if (!C_EMAIL_PASWD) echo(" e una password"); ?> per voi digitandolo nello spazio appropriato. Lo pseudonimo che sceglierete sar&agrave; il nome che comparir&agrave; automaticamente nella chat. Non pu&ograve; contenere spazi, virgole o backslash (\).
<?php if (C_NO_SWEAR) echo(" Non pu&ograve; contenere \"parole vietate\"."); ?>
	...
	<LI>Il campo della lingua pu&ograve; aiutare altri utenti in future discussioni: essi sapranno quale lingue voi potere capire.
...
desiderate che sia visibile il vostro indirizzo email, lasciate la casella non spuntata.
	<LI>Dopo, premete il bottone Registrati e verr&agrave; creato il vostro account. Se volete interrompere la registrazione in qualunque momento, premete il tasto Chiudi.
</UL>
<P>
...
if ($Ver == "H")
{
	...
	<P>
	Un segnale che rappresenta lo stato della vostra connessione &egrave; situato all'angolo destro in alto dello schermo. Pu&ograve; essere di tre forme:
	<P>
	<UL>
		<LI><IMG SRC="images/connectOff.gif" WIDTH=13 HEIGHT=13 BORDER=0 ALT="Nessuna connessione"> Quando non &egrave; richiesta una connessione ;
		<LI><IMG SRC="images/connectOn.gif" WIDTH=13 HEIGHT=13 BORDER=0 ALT="Connessione in corso"> Quando c'&egrave; una connessione in corso ;
		<LI><IMG SRC="images/connectError.gif" WIDTH=13 HEIGHT=13 BORDER=0 ALT="Connessione fallita"> Quando fallisce una connessione.
	</UL>
	<P>
	Nel terzo caso, cliccando sul pulsante "rosso" lancer&agrave; un nuovo tentativo di connessione.
	<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
	<P>
	<hr />
	<?php
...
<TR>
	...
	<UL>
		...
		<LI>alien (il cui genere &egrave; sconosciuto), Jezek2 e Caridad sono utenti registrati senza alcun privilegio extra per la stanza phpMyChat;<br /><br />
		<LI>lolo &egrave; semplicemente un utente non registrato.
	</UL>
	</TD>
</TR>
</TABLE>
...
<P>
invier&agrave; il messaggio "arrivederci!" nel frame principale dopodich&egrave; si uscir&agrave;.
...
<UL>
	...
	?>
	<LI>Il <B>comando Notify</B> vi permette di abilitare/disabilitare l'opzione con cui si possono vedere i messaggi di notifica quando gli altri utenti entrano/escono dalla stanza della chat. Di base questa opzione &egrave; <?php echo(C_NOTIFY ? "abilitata" : "disabilitata"); ?> e le notifiche <?php echo(C_NOTIFY ? "saranno" : "non saranno"); ?> viste.<br />Digitate "/notify" senza virgolette.
	<P>
	<LI>Il <B>comando Timestamp</B> vi permette di abilitare/disabilitare l'opzione con cui viene visualizzato l'ora del messaggio che viene postato prima di ogni messaggio e l'ora del server nella barra di stato. Di norma questa opzione &egrave; <?php echo(C_SHOW_TIMESTAMP ? "abilitata" : "disabilitata"); ?>.<br />Digitate "/timestamp" senza virgolette.
	<P>
	<LI>Il <B>comando Refresh</B> vi permette di aggiustare il ritmo con cui i messaggi digitati sono ri-visualizzati sul vostro schermo. Il ritmo standard &egrave; correntemente di <?php echo(C_MSG_REFRESH); ?> secondi. Per cambiare il ritmo digitate "/refresh n" senza virgolette dove n &egrave; il tempo in secondi del nuovo ritmo di ri-visualizzazione.
	<P>
	<I>Per esempio:</I> /refresh 5
	<P>
	cambier&agrave; il ritmo a 5 secondi. *Attenzione, se n &egrave; settato a meno di 3 secondi, la ri-visualizzazione viene resettata proprio per non fare la ri-visualizzazione (utile se volete leggere molti messaggi senza essere disturbati)!*
	<P>
	<?php
	if ($Ver == "L")
	{
		?>
		<LI>Il <B>comando Show</B> vi permette di aggiustare il numero di messaggi visualizzabili sul vostro schermo. per cambiare il valore di base, digitate "/show n" senza virgolette dove n &egrave; il numero di messaggi da visualizzare.
		<P>
		<I>Per esempio:</I> /show 50
		<P>
		far&agrave; in modo che i 50 messaggi piů nuovi siano visibili sul vostro schermo. Se tutti i messaggi non possono essere visualizzari dentro la finestra della chat, apparir&agrave; una barra di scorrimento sul lato destro della schermata dei messaggi.</UL>
		<?php
	}
	else
	{
		?>
		<LI>I <B> comandi Show e Last</B> vi permettono di ripulire lo schermo e di visualizzare gli ultimi <I>n</I> messaggi inviati sul vostro schermo. Digitate /show n" o "/last n" senza virgolette dove n &egrave; il numero di messaggi da visualizzare.
		<P>
		<I>Per esempio:</I> /show 50 o /last 50
		<P>
		ripulirer&agrave; il frame dei messaggi e far&agrave; in modo che i 50 messaggi piů nuovi siano visibili sul vostro schermo. Se tutti i messaggi non potranno essere visualizzati all'interno della schermata dei messaggi, apparir&agrave; una barra di scorrimento sul lato destro di quest'ultima.</UL>
		<?php
	};
	?>
	<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
...
<P>
Una volta all'interno di una stanza di chat, potete avviare il popup di aiuto cliccando sull'immagine <IMG SRC="images/helpOff.gif" WIDTH=30 HEIGHT=20 BORDER=0 ALT="Aiuto"> che si trova appena prima del frame di invio messaggi. Potete anche digitare i <B>comandi "/help" o "/?"</B> nel campo invio messaggi.
<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
<P>
<P>
<!-- Avatar System Start. -->
...
<P>Clickando sull'avatar di una persona nella sezione dei messaggi, aprir&agrave; una finestra pop up con il profilo della persona (vedi il comando <A HREF="#whois">/whois</A>).
Clickando sul proprio avatar nella lista utenti richiamer&agrave; il comando /profile , se sei registrato.
Se non sei registrato, clickando sul proprio avatar (&egrave; di default), aprir&agrave; un avviso per incoraggiarti a registrarti.
...
if (C_HTML_TAGS_KEEP != "none")
{
	...
	<P>
	<I>Per esempio</I>, &LT;B&GT;questo testo&LT;/B&GT; produrr&agrave; <B>questo testo</B>.
	<P>
	Per creare un collegamento ipertestuale per un indirizzo email o un URL, digitate l'indirizzo (senza alcun tag HTML). Il collegamento ipertestuale verr&agrave; creato automaticamente.
	<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
	<P>
	<P>
	<hr />
...
<P>
<b><?php echo(L_COL_HELP_SUB3); ?></b><br /><?php echo(L_COLOR_HEAD_SETTINGS); ?><br /><?php if (COLOR_FILTERS) echo(L_COLOR_HEAD_SETTINGSa."<br />"); ?><u><?php echo(L_COL_HELP_USER_STATUS); ?></u> = <b><?php if ($CookieStatus == "a") echo("Amministratore"); elseif ($CookieStatus == "m") echo("Moderatore"); elseif ($CookieStatus == "u") echo("Ospite (Normal)"); else echo("Registrato (Normal)");?></b><br /><?php if (COLOR_FILTERS) echo("<br />".L_COL_HELP_P3."<br />"); ?><?php echo(L_COL_HELP_P3a); ?>
<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
<hr />
<!-- Color Input Box mod by Ciprian end -->
<P>
...
<P>
Notate che potete inserire pi&ugrave; di un nomeutente nel comando /invite (per es. "/invite Jack,Helen,Alf"). Essi devono essere divisi da una virgola (,) senza spazi.
<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
<P>
<hr />
...
if (C_VERSION == 2)
{
	...
	<P>
	creer&agrave; una nuova stanza privata (dando per scontato che una pubblica con lo stesso nome non sia gi&agrave; stata creata) chiamata MiaStanza e vi porter&agrave; all'interno di essa.
	<P>
	I nomi di stanze non possono contenere spazi o backslash (\).<?php if (C_NO_SWEAR) echo(" Non pu&ograve; contenere parole vietate."); ?>
	<?php
}
?>
<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
...
<P>
<FONT SIZE="+1"><B>Modificare il proprio profilo all'interno della chat:</B></FONT>
<P>
Il <B>comando Profile</B> crea una finestra pop-up separata nella quale &egrave; possibile modificare il proprio profilo utente e modificare tranne che per il cotro nome utente e password (dovrete usare il link all'inizio della pagina d'ingresso.<br />Digitate /profile
<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
<P>
<hr />

<P>
<FONT SIZE="+1"><B>Richiamare l'ultimo messaggio o comando che avete inviato:</B></FONT>
<P>
...
<P>
<FONT SIZE="+1"><B>Rispondere ad un utente specifico:</B></FONT>
<P>
Cliccando una volta sul nome di un utente nella vostra stanza chat (alla destra dello schermo) far&agrave; che appaia "nomeutente>" nel vostro campo invio messaggi. Questa caratteristica permette di dirigere facilmente un messaggio pubblico ad un utente, forse in risposta a qualcosa che egli/lei aveva postatao sopra.
<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
<P>
<hr />
...
<P>
<I>Per esempio, dove Jack &egrave; il nome utente:</I> /msg Jack Io sono qua, come stai?
<P>
Il messaggio apparir&agrave; a Jack e a voi stessi, ma nessun altro utente lo vedr&agrave;.
<P>
Osservate che cliccando su un nick di qualcuno che ha scritto nel frame principale, verr&agrave; aggiunto automaticamente questo comando nel campo invio messaggi.
<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
<P>
<hr />
...
<P>
<I>Per esempio:</I> Se Jack manda il messaggio "/me sta bevendo una tazza di caff&egrave;" il messaggio che comparir&agrave; sar&agrave; "<B>* Jack</B> sta bevendo una tazza di caff&egrave;".
<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
<P>
<hr />
...
<P>
Da questo momento in poi, nessun messaggio proveninete dall'utente Jack sar&agrave; visualizzato sullo schermo.
<P>
Per avere la liste degli utenti i cui messaggi sono ignorati, basta che digitate il <B>comando "/ignore"</B> senza virgolette.
<P>
Per ripristinare la visualizzazione dei mesaggi di un utente ignorato, digitate il <B>comando "/ignore - nomeutente"</B> senza virgolette dove '-' &egrave; un trattino.<P>
<P>
<I>Per esempio:</I> /ignore - Jack
<P>
...
Se non specificate un nomeutente dopo hyphen, la vostra 'lista degli ignorati' sar&agrave; ripulita.
<P>
Notate che potete inserire pi&ugrave; di un nomeutente nel comando /ignore (per es. "/ignore Jack,Helen,Alf" oppure "/ignore - Jack,Alf"). Essi devono essere divisi da una virgola (,) senza spazi.
<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
<P>
<hr />
...
<P>
dove 'Jack' &egrave; il nome utente. Questo comando creerer&agrave; una finestra popup separata che mostrer&agrave; le informazioni pubblicamente disponibili per quel utente.
<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
<P>
<hr />
...
if (C_SAVE != "0")
{
	...
	<P>
	dove '5' &egrave; il numero di messaggi da salvare. Se n non &egrave; specificato, tutti i messaggi disponibili inviati nella stanza corrente saranno presi in considerazione.
	<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
	<P>
	<hr />
	<?php
...
<P>
L'amministratore pu&ograve; inviare un annuncio a tutti gli altri utenti in qualunque stanza essi stiano chattando con il <B>comando announce</B>.
<P>
<I>Per esempio:</I> /announce La chat sar&agrave; disattivata per manutenzione stasera alle 20.00.</I>
<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
<P>
C'&egrave; un altro utile comando per annuncio da atteggiare nelle chat; l'amministratore o il moderatori nella stanza pu&ograve; anche inviare un annuncio a tutti gli utenti della stanza corrente o tutte le stanze con il <B>comando room</B>.
<P>
...
<P>
I moderatori possono cacciare un utente, mentre l'amministratore pu&ograve; cacciare un utente o un moderatore con il <B>comando kick</B>. A parte l'amministratore, l'utente da allontanare deve stare nella stanza corrente.
<P>
<I>Per esempio, se Jack &egrave; il nome dell'utente da cacciare via:</I> /kick Jack</I> o <I>/kick Jack motivo della scalciata</I>. Il "motivo della scalciata" pu&ograve; essere qualsiasi testo e.g. "per spamming".
<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
<P>
<hr />
...
if (C_BANISH != "0")
{
	...
	<P>
	I Moderatori possono espellere un utente e l'amministratore pu&ograve; espellere un utente o un moderatore con il comando <b>ban</b>.<BR>
	L'amministratore pu&ograve; bandire un utente da un altra area nella quale st&agrave; chattando. Pu&ograve; anche bandire per sempre un utente e dall'area dove si trova e dalla chat intera utilizzando l'asterisco '<B>&nbsp;*&nbsp;</B>' che deve essere inserito prima del nick dell'utente che si vuole espellere.
	<P>
	<I>Per esempio</I>, se Jack &egrave; il nome dell'utente da espellere: <I>/ban Jack</I>, <I>/ban * Jack</I>, <I>/ban Jack motivo dell'espulsione</I> o <I>/ban * Jack motivo dell'espulsione</I>. Il "motivo dell'espulsione" pu&ograve; essere qualsiasi testo e.g. "per spamming".
	<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
	<P>
	<hr />
	<?php
...
<P>
<I>Per esempio</I>, se Jack &egrave; il nome dell'utente da promuovere:<I> /promote Jack</I>
<P>
Solo l'amministratore pu&ograve; accedere al comando opposto (ridurre un moderatore a semplice utente) usando il <B>comando demote</B>.
<P>
<I>Per esempio</I>, se Jack &egrave; il nome del moderatore da degradare: <I>/demote Jack</I> o <I>/demote * Jack</I> (lo degrader&agrave; dalla stanza corrente o tutte le stanze).
<br /><P ALIGN="right"><A HREF="#top">Torna all'inizio</A></P>
<P>
</BODY>
</HTML>
