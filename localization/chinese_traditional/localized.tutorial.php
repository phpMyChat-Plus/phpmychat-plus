...
<P>
有許多不同的方法來定制聊天的外觀。要更改設置，只需到您的文字方塊中鍵入相應的命令，並按下Enter / 傳送 鍵。
<P>
<UL>
	<?php
	if ($Ver == "H")
	...
	?>
	<LI>這 <B>Notify 指令</B> 允許您打開或關閉看到的告示，當其他用戶進入或退出聊天室的選項。默認情況下此選項 <?php echo(C_NOTIFY ? "on" : "off"); ?> 和 告示 <?php echo(C_NOTIFY ? "will" : "won’t"); ?> 可見。<br /> 鍵入 "/notify" 不帶引號。
	<P>
	<LI>這 <B>Timestamp 指令</B> 允許您打開或關閉看到在狀態欄上的每個消息和服務器時間發布消息之前的選項。默認情況下此選項 <?php echo(C_SHOW_TIMESTAMP ? "on" : "off"); ?>.<br />鍵入 "/timestamp" 不帶引號。
	<P>
...
if (C_USE_SMILIES)
{
	...
	<P>
	<I>例如</I>, 發送文本 "Hi Jack :)" 不帶引號 將顯示訊息 <B>Hi Jack</B> <IMG SRC="images/smilies/smile1.gif" WIDTH=15 HEIGHT=15 ALT=":)"> 在主畫面中。
	<P ALIGN="center">
...
{
?>
點擊在主畫面中說話的人的暱稱，會自動添加 / 或 /WISP 命令輸入字段的消息。
<?php
}
else
{
...
<P>
要忽略由其他用戶發言的所有訊息，鍵入這個 <B>指令 "/ignore username"</B> 不帶引號。
<P>
<I>例如：</I> /ignore Jack
<P>
...
<P>
恢復一個被忽略的用戶的消息顯示，鍵入這 <B>指令 "/ignore - username"</B> 不帶引號, 其中 "-" 是一個連字符（減號）。<P>
<P>
<I>例如：</I> /ignore - Jack
<P>
