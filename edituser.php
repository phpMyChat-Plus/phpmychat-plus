		...
		<TR>
		  <TD ALIGN="RIGHT" VALIGN="TOP" NOWRAP="NOWRAP"><?php echo(L_SET_2); ?> :</TD>
			<TD VALIGN="TOP">
				...
				<INPUT TYPE="text" NAME="U" SIZE=15 MAXLENGTH=15<?php echo($field_errorU ? " style=\"background-color: #FF0000;\"" : ""); ?> VALUE="<?php echo(htmlspecialchars(stripslashes($U))); ?>"<?php if ($done) echo(" READONLY"); if (isset($LIMIT) && $LIMIT) echo(" DISABLED"); ?>>&nbsp;<?php if (!$done) echo("<SPAN CLASS=\"error\">*</SPAN>"); ?>
				<?php
...
?>
		...
		<?php
			...
			while($langname = $languageDirectories->read())
			{
				...
				{
					list($langkey) = file('./'.$ChatPath.'localization/'.$langname.'/regex.txt');
					$AvailableLanguages[$langkey] = $langname;
				};
			};
		...
		<?php
			...
			while(list($key, $langname) = each($AvailableLanguages))
			{
				...
				$i++;
				?>
				<OPTION VALUE="<?php echo $langname ?>" <?php if($SLANG==$langname){ echo " selected"; $langnamesel = $langname; } ?>><?php echo ($FLAG_NAME); ?></OPTION>
					<?php
			};
			...
			?>
	    </SELECT>&nbsp;<img style="vertical-align:middle" id="flagToSwap" src="<?php echo("./".$ChatPath."localization/".$langnamesel."/images/".(C_FLAGS_3D ? $flagsel_3d : $flagsel)); ?>" /><?php if (!$done && $SLANG != $L) echo("&nbsp;<SPAN CLASS=\"error\">*</SPAN>"); ?>
			</TD>
		</TR>
		<TR>
