...
// Avatar System end.
		...
		else
		{
			...
			elseif ($User == "SYS image")
			{
        		...
        		$NewMsg .= "<font class=\"notify\">".$Pic." ".$Dest.":</font></td><td valign=\"top\"><a href=".$Message." onMouseOver=\"window.status='".sprintf(L_CLICK,L_FULLSIZE_PIC).".'; return true\" title='".sprintf(L_CLICK,L_FULLSIZE_PIC)."' target=_blank><img src=".$Message." width=".$imgSize[0]." height=".$imgSize[1]." border=0 alt='".sprintf(L_CLICK,L_FULLSIZE_PIC)."'></a>".$Resized."</td></tr></table>";
      		}
