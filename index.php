...
if ($S)
{
	...
	echo("<TD><SPAN style=\"color:blue; background-color:black;\"><b>Debug data:</b><SPAN style=\"color:yellow;\"><br />Admin name: <b>".C_ADMIN_NAME."</b><br />Admin email: <b>".C_ADMIN_EMAIL."</b><br />App name: <b>".APP_NAME."</b><br />Chat name: <b>".C_CHAT_NAME."</b><br />App version: <b>".APP_VERSION.APP_MINOR."</b><br />Hosting Server IP: <b>".$_SERVER['SERVER_ADDR'].(@fsockopen("gravatar.com", 80, $errno, $errstr, 2) ? "</b>" : "<br /><font color=red>Cache not allowed - cannot get access to gravatar.com!</font></b>")."<br />Php server version: <b>".(!version_compare(phpversion(),'5','>=') ? "<font color=red>".phpversion()." - Cache not allowed</font>" : phpversion())."</b><br />allow_url_fopen: <b>".(!(ini_get("allow_url_fopen")) ? "<font color=red>".L_DISABLED." - Cache not allowed</font>" : L_ENABLED)."</b><br />allow_url_include: <b>".(!(ini_get("allow_url_include")) ? "<font color=red>".L_DISABLED."</font>" : L_ENABLED)."</b><br />fsockopen: <b>".(!(@fsockopen("ciprianmp.com", 80, $errno, $errstr, 2) && @fsockopen("phpmychat.svn.sourceforge.net", 80, $errno, $errstr, 2)) ? "<font color=red>".L_DISABLED." - Update checking not allowed</font>" : L_ENABLED)."</b><br />file_get_contents: <b>".(!(function_exists("file_get_contents")) ? "<font color=red>".L_DISABLED." - Cache not allowed</font>" : L_ENABLED)."</b><br />MySQL server version: <b>".mysql_get_server_info()."</b></SPAN></SPAN></TD>");
	@mysql_close($conn);
}
