...
if($DbLink->num_rows() != 0)
{
	...
	if(C_EN_STATS)
	{
		...
		$DbLink->query("UPDATE ".C_STS_TBL." SET seconds_away=seconds_away+($curtime-last_away), longest_away=IF($curtime-last_away < longest_away, longest_away, $curtime-last_away), last_away='' WHERE stat_date=FROM_UNIXTIME(last_away,'%Y-%m-%d') AND room='$R' AND username='$U' AND last_away!='0'");
	}
		$kicked = 3;
	}
...
if(C_CHAT_BOOT)
{
	...
	if($DbLink->affected_rows() > 0)
	{
		...
		if(C_EN_STATS)
		{
			...
			$DbLink->query("UPDATE ".C_STS_TBL." SET seconds_away=seconds_away+($curtime-last_away), longest_away=IF($curtime-last_away < longest_away, longest_away, $curtime-last_away), last_away='' WHERE stat_date=FROM_UNIXTIME(last_away,'%Y-%m-%d') AND room='$m_room' AND username='$U' AND last_away!='0'");
		}
	$DbLink->clean_results();
